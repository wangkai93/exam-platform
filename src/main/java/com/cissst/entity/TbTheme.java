...
public class TbTheme implements java.io.Serializable {
	...
	private List<TbThemeKey> tbThemeKey=new ArrayList<TbThemeKey>();
	
	
	public List<TbThemeKey> getTbThemeKey() {
		Set<TbThemeKey>set=tbThemeKeies;
		for ( TbThemeKey themeKey :set) {
			if("A".equals(themeKey.getThekeyId())){
				tbThemeKey.add(themeKey);
			}
		}
		for ( TbThemeKey themeKey :set) {
			if("B".equals(themeKey.getThekeyId())){
				tbThemeKey.add(themeKey);
			}
		}
		for ( TbThemeKey themeKey :set) {
			if("C".equals(themeKey.getThekeyId())){
				tbThemeKey.add(themeKey);
			}
		}
		for ( TbThemeKey themeKey :set) {
			if("D".equals(themeKey.getThekeyId())){
				tbThemeKey.add(themeKey);
			}
		}
		for ( TbThemeKey themeKey :set) {
			if("E".equals(themeKey.getThekeyId())){
				tbThemeKey.add(themeKey);
			}
		}
		return tbThemeKey;
