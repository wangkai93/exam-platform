...
public class TbExamCommit implements java.io.Serializable {
	...
	private String col2;
	private String col3;
	private List<String>commitKeyL=new ArrayList<String>();
	
	
	public List<String> getCommitKeyL() {
		if("—°‘ÒÃ‚".equals(tbTheme.getTbThemeType().getThetypeName())){
			String temp= commitKey.trim();
			for (int i = 0; i <temp.length(); i++) {
				String keyId=temp.substring(i, i+1);
				commitKeyL.add(keyId);
			}
		}
		return commitKeyL;
	}
	public void setCommitKeyL(List<String> commitKeyL) {
		this.commitKeyL = commitKeyL;
	}
	public TbExamCommit() {
	}
