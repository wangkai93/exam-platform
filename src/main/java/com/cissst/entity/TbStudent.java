...
public class TbStudent implements java.io.Serializable {
	...
	private String col2;
	private String col3;
	private Set<TbStudentGrade> tbStudentGrades = new HashSet(0);
	...
	public List getGrades() {
		Set<TbPlan> tbPlants = this.tbClass.getTbPlans();
		grades  = new ArrayList();
		for (TbPlan tbPlan : tbPlants) {	
		   int i = 0;
		   for (TbStudentGrade tsg : tbStudentGrades) {
				if(tsg.getTbPlan()!=null&&tsg.getTbPlan().getPlanId()==tbPlan.getPlanId()){
					grades.add(tsg);
					i++;
					System.out.println("##################");
					break;
		   ...
		   if(i==0){
			   TbStudentGrade temp = new   TbStudentGrade();
				grades.add(temp);
		   }
